...
- Postgres not exposed outside the cluster — only reachable by `auth-service`, `gateway`, and `ml-service`, all inside the same k3s network
...
- Deployment philosophy — still one EC2 instance, still cost-minimized (k3s instead of EKS, self-hosted Postgres instead of RDS, MongoDB Atlas free tier)
...
- [x] EC2 `t4g.medium` (ARM/Graviton) up with a 15GB gp3 root volume, Elastic IP attached, k3s installed (`curl -sfL https://get.k3s.io | sh -`)
- [ ] Postgres provisioned on Neon or Supabase free tier, connection string saved as a k3s Secret (no self-hosted Postgres pod/PVC needed)
