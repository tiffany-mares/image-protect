...
- [x] Dockerfile per service (ARM64, built natively on the EC2 host — no registry; imported into k3s containerd via `docker save | k3s ctr images import`)
- [x] k8s manifests (`k8s/ml.yaml`, `auth.yaml`, `gateway.yaml`): Deployment + ClusterIP Service each; Traefik Ingress on the gateway only, host `inkshield-api.duckdns.org`, Let's Encrypt TLS (`k8s/traefik-acme.yaml`)
- [x] Secrets applied: `jwt-signing-key`, `db-credentials`, `mongo-uri` + configmap `app-config`. (Adapted from the doc's `ses-credentials`/`s3-credentials`: SES + S3 use the EC2 instance role `ImageProtectEC2Role` — `s3:PutObject/GetObject` + `ses:SendEmail` — so no static AWS secrets live in the cluster.)
- [x] S3 already private + presigned since Phase 1 (gateway presigns for gallery/dashboard reads at request time); CORS pinned on the gateway to `https://image-protect-beta.vercel.app`
- [x] Frontend live on Vercel (`https://image-protect-beta.vercel.app`) with `VITE_API_URL` = the gateway ingress; needed a Nitro preset fix (`vercel` when `VERCEL=1`, else `node-server`) since Phase 1 had hard-pinned `node-server`
